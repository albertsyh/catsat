const CatThumb = ({ cats, onToggle, view, close }) => {
  return cats.map((cats) => (
    <div
      class="moreContainer"
      key={cats.id.toString()}
      style={{ backgroundColor: cats.background_color }}
    >
      <div className="row catDetails">
        <div
          className="col-12 catThumbnail text-end pt-2"
          style={{ backgroundImage: "url(" + cats.image_url + ")" }}
        >
        ...
        <div className="col-4">
          <button
            className={cats.liked ? "liked" : ""}
            onClick={() => onToggle(cats.id)}
          >
            <i
              className={
                cats.liked ? "fas fa-heart likedHeart" : "far fa-heart"
              }
              style={{ color: "#F46D5C" }}
            ></i>
            {cats.liked ? " Liked" : " Like"}
          </button>
        </div>
